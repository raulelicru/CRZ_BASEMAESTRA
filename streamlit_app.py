...
VERSION = "2026.07.01-v · Mora: cobrador de ZONAS por ZONA (valor distinto de ZONA)"
...
with tab_val:
    ...
    st.subheader("🔎 Distribución por campaña")
    ...
    if buscar:
        ...
        if len(en_aud):
            ...
            st.info("No aparece en la base ni en auditoría: no existe con ese formato "
                    "en las fuentes, o su dama tiene una campaña más reciente "
                    "(se conserva solo la más reciente por NO_DAMA).")

    st.caption("Registros por campaña **en la Base Maestra**")
    st.dataframe(dist, width="stretch", hide_index=True)
    if not aud_camp.empty:
        st.caption("Campañas en **auditoría** (excluidas / descartadas)")
        st.dataframe(aud_camp, width="stretch", hide_index=True)
